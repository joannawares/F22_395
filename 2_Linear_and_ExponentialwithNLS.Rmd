...
date: Due September 16 at 11:59 PM
output: pdf_document
urlcolor: blue
---
...
Load the data with the `read_csv()` function. It will be a dataframe with attributes "Time" and "NewCases."
...
and new cases on the y-axis like we did in the previous exercise.
...
Plot the data and the best fit line together. You can do this in a bunch of ways. One is to use `stat_smooth` another is to use geom_abline. Let's try geom_abline so we can really see what we are doing. Here is an example [https://www.rdocumentation.org/packages/ggplot2/versions/0.9.0/topics/geom_abline](https://www.rdocumentation.org/packages/ggplot2/versions/0.9.0/topics/geom_abline) The line "geom_abline(aes(intercept=a, slope=b), data=df)" is a good model of how to do this.

```{r linear-plot2, warning=FALSE}
```
...
Now we will try another method for fitting the exponential that was described in the post above. We will try to use nonlinear least squares, nls, to fit our exponential so the formula is `formula = TotalCases ~ exp(r*Time + b)` and use the values found above as the starting values. Assign this to expMod.
```{r}
```

Now let's plot this fit with the original data
```{r}
```

Question: How does this fit from nls compare with the loglinear fit? 

**Answer**

## Early Epidemic Data

Now let's do the same exercise, except for the CDC's Monkey Pox data. I've created a file for this in CDCMonkeyPox.csv. It's probably easiest if you save this file with a different name. Then, all we want to do is use the new data file at the beginning. You see from the first few graphs that the linear/exponential split seems to happen around day 65 and that the first 7 days of data should be discarded later on. You won't need to make many changes to your document, just adjust for these differences. Knit the new file with the new data.

Question: Do you think Monkey Pox had a similar shaped trajectory as COVID? Why? What differences did you notice?
...
Question: Summarize the controversy here? 
